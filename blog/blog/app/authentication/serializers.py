...
class LoginSerializer(ModelSerializer):
    ...
    def validate(self,data):
        ...
        if password is None:
            raise serializers.ValidationError('Passord is required to log in.')

        user = authenticate(username=email, password=password)

        if user is None:
            raise serializers.ValidationError('A user with this credetials was Not Found.')
...
class UserSerializer(ModelSerializer):
    ...
    dp=serializers.CharField(source='profile.dp',read_only=True)

    class Meta:
        model = User
        fields = ('email','username','password','token','profile','status','dp',)
